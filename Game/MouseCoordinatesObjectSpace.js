...
// Define a new class for displaying the mouse coordinates in object space
class MouseCoordinatesObjectSpace extends GameObject {
  constructor(ball, screenWidth, screenHeight, marginX = 20, marginY = 20) {
  ...
  // Attach a listener to the mousemove event to update the mouse position in object space
  trackMouse() {
    document.addEventListener("mousemove", (event) => {
      this.mouseX = event.clientX - this.ball.x;
      this.mouseY = this.ball.y - event.clientY;
    });
  }
...
export default MouseCoordinatesObjectSpace;
